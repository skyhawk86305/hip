...
RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::search ENTERED"

    session[:hc_group_id]=params[:deviation_search][:hc_group_id]
    session[:deviation_search]=params[:deviation_search]
#05-21-2013  Added group_names to parameters
    session[:group_names]=params[:group_names]

RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::Calling DeviationSearch.search"

    result = DeviationSearch.search(params[:deviation_search], 0, 1)

RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::Returned from DeviationSearch.search"
...
#05-21-2013  Get the group name from the list of HC_Cycle_Group names selected

   session[:group_names].each do |group_name|    

 RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::Search group_name is #{group_name}"

#05-21-2013 old    group_name = HcGroup.find(session[:deviation_search][:hc_group_id]).group_name
    @count = count
  ...
  def set_hc_group_id
      RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::set_hc_group_id ENTERED"
      params[:deviation_search][:hc_group_id]=params[:hc_group_id]
#      RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController::hc_group_id #{ params[:deviation_search][:hc_group_id] }"
  end
  ...
  def upload
    ...
    begin
      ...
      @errors << OfflineSuppressions::SuppressionCsv.validate('ic',file,group_name,nil,current_org_id) if @errors.empty?
      @errors.flatten!
      @errors.compact!
