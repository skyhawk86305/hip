...
# skip_before_filter :authorization_filter, :only =>[:search]
 ...
 RAILS_DEFAULT_LOGGER.debug "OfflineSuppressionsController ENTERED "

     @params = params
#05-15-2013
 
    if @params[:deviation_search][:hc_group_id].kind_of?(Array)
      group_id_list = @params[:deviation_search][:hc_group_id].map {|group_id| group_id.to_i}
      params[:deviation_search][:hc_group_id] = group_id_list
    else
      group_id_list = [@params[:deviation_search][:hc_group_id].to_i]
      params[:deviation_search][:hc_group_id] = group_id_list
    end

  RAILS_DEFAULT_LOGGER.debug "OutOfCycle::OfflineSuppressionsController @params[:deviation_search][:hc_group_id] is #{ params[:deviation_search][:hc_group_id] }"
